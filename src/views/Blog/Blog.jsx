...
export default function Blog() {
    ...
    function supprimer(postId) {

        const confirmer = window.confirm("Êtes-vous sûr de vouloir supprimer cette publication ?");
        if (!confirmer) return;

        axios
            .delete(`/publications/${postId}`)
            .then(() => {
                setPublications(publis => publis.filter((p) => p.id_publication !== postId))
                alert("Publication supprimée avec succès");
            })
            .catch((erreur) => {
                console.error("Erreur suppression :", erreur);
                setErreur("Impossible de supprimer ce post pour le moment.");
            });
                        ...
                        <div className="card blog-card w-100 shadow-sm" key={ind} style={{ minWidth: "45%" }}>
                            <div className="row align-items-center">
                                ...
                                <div className="col-2 d-flex flex-column justify-content-center align-items-center gap-4">
                                    ...
                                    {/* Bouton modifier */}
                                    {user?.id == p?.id_users &&
                                        <Link to={`/modif/${p.id_publication}`} className="btn btn-icon"> <svg xmlns="http://www.w3.org/2000/svg" width="35" height="35" fill="white" className="bi bi-pencil-square" viewBox="0 0 16 16">
                                    ...
                                    {/* bouton supprimer */}
                                    {(user?.id == p?.id_users || user?.role == "admin") &&

                                        <button className="btn tool" onClick={() => supprimer(p.id_publication)}>
                                            <svg xmlns="http://www.w3.org/2000/svg" width="35" height="35" fill="red" className="bi bi-ban" viewBox="0 0 16 16">
